...
def test_zonage_trouver_zones():
    ...
    assert (
        resultat == zones_attendues
    ), f"Erreur : zones attendues {zones_attendues}, obtenu {resultat}"
...
def test_zonage_trouver_zones_chemins():
    ...
    assert (
        chemins_obtenus == chemins_attendus
    ), f"Erreur : chemins attendus {chemins_attendus}, obtenu {chemins_obtenus}"

def test_zonage_trouver_zone_avec_mere():
    point_dans_zones_fille = P(0.5, 0.5)
    point_hors_zone = P(100, 100)

    # Zonage mère et fille définis
    zones_fille = pytest.zone_1_0_1
    zonage_mere = Zonage(nom="Zonage Mère", zones=[pytest.zone_0_0_3])
    zonage_fille = Zonage(nom="Zonage Fille", zones=[zones_fille], zonage_mere=zonage_mere)

    # Point dans la zone fille
    assert (
        zonage_fille.trouver_zone(point_dans_zones_fille) == zones_fille
    ), "Erreur pour la recherche dans zone fille"
...
def test_zonage_trouver_zone_chemin():
    ...
    assert (
        zonage_fille.trouver_zone_chemin(point_dans_zones_fille) == chemin_attendu
    ), "Erreur sur le chemin attendu"

    # Chemin pour un point hors zone
    assert (
        zonage_fille.trouver_zone_chemin(point_hors_zone) == ""
    ), "Erreur pour le chemin hors zone"
